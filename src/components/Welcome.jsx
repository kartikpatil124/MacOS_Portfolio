...
const setupTextHover = (container, type) => {
    ...
    const handleMouseMove = (e) => {
        ...
        letters.forEach((letter) => {
            ...
            animationLetters(letter, min + (max - min) * intensity);
        });
    };

    const handleMouseLeave = () => letters.forEach((letter) => 
        animationLetters(letter, base, 0.3)
    );
...
setupTextHover(subtitleRef.current, "subtitle");
  ...
  return (
    <section id="welcome">
        <p ref={subtitleRef}>
            {renderText("Hey I'm Kartik! Welcome to my" , "text-3xl font-georama", 100)} </p>
        <h1 ref={titleRef} className="mt-7">{renderText("portfolio", "text-9xl italic font-georama")}</h1>

        <div className='small-screen'>
            <p>This Portfolio is Built for Desktop/Tablet Screen Size.</p>
        </div>
    </section>
